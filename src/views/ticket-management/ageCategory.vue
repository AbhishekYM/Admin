...
import { useAgeCategory } from '/src/store/ageCategory.js';

const store = useAgeCategory();
...
// const totalUsers = computed(() => store.ageCategory.length);
const searchQuery = ref("");

const filteredAgeCategorys = computed(() => {
    let ageCategory = store.ageCategory;
    if (searchQuery.value.trim()) {
        const query = searchQuery.value.toLowerCase();
        ageCategory = ageCategory.filter(ageCategory => {
            return (
                ageCategory.name.toLowerCase().includes(query) 
              );
        });
    }
    const start = (currentPage.value - 1) * perPage.value;
    const end = start + perPage.value;
    return ageCategory.slice(start, end);
});
const page = ref(1);
const itemsPerPage = ref(10);
const paginatedAgeCategorys = computed(() => {
    const startIndex = (page.value - 1) * itemsPerPage.value;
    const endIndex = startIndex + itemsPerPage.value;
    return filteredAgeCategorys.value.slice(startIndex, endIndex);
});

// Calculate total pages
const totalPages = computed(() => {
    return Math.ceil(filteredAgeCategorys.value.length / itemsPerPage.value);
});
...
<Layout>
    <div class="d-flex flex-column gap-2">
        <BBreadcrumb class="p-3 py-2 bg-light">
            ...
            <BBreadcrumbItem href="#general">Age Category</BBreadcrumbItem>
        </BBreadcrumb>
    </div>
    <BCard>
        <BCardHeader>
            <BRow class="align-items-center g-2">
                <BCol lg="3" class="me-auto">
                    <BCardTitle title-tag="h6" class="mb-0">Age Category List</BCardTitle>
                </BCol>
                <BCol xl="2" md="3">
                    <div class="search-box">
                        <input type="text" class="form-control search" placeholder="Search for Age Category" v-model="searchQuery">
                ...
                <BCol md="auto">
                    <div class="hstack gap-2">
                        ...
                        <BButton variant="primary" @click="store.modalShow17 = !store.modalShow17"> Add Age Category </BButton>
                    </div>
                </BCol>
            </BRow>
        </BCardHeader>
      ...
      <BButton type="button" variant="secondary" class="w-sm" @click="store.showDeleteModal = false">Cancel</BButton>
          ...
          <BForm>    
            <BRow class="align-items-center g-4" >
                <BCol lg="6" class="me-auto">
                    <label for="Designationname" class="form-label">Age Category name <span class="text-danger">*</span></label>
            ...
            <div class="modal-footer">
                ...
                <BButton  @click="store.storeAgeCategorys()"  variant="primary">Save Changes</BButton>
            </div>
          </BForm>
        </BModal>
        <BRow id="invoiceList">
        ...
        <BCardBody class="mt-3">
            <div class="table-responsive table-card">
                <BTableSimple class="table-centered align-middle table-custom-effect table-nowrap mb-0">
                    <BThead class="table-light">
                        <BTr>
                            ...
                            <BTh scope="col" class="sort cursor-pointer" data-sort="user_name">Age Category Name</BTh>
                            <BTh scope="col" class="sort cursor-pointer" data-sort="action">Action</BTh>
                        </BTr>
                    </BThead>
                    <BTbody>
                        <BTr v-for="(ageCategory, index) in paginatedAgeCategorys" :key="index">
                            <BTd class="user_id">{{ index + 1 }}</BTd>
                            <BTd class="user_name">{{ ageCategory.name }}</BTd>
                              <BTd>
                                <ul class="d-flex gap-2 list-unstyled mb-0">
                                    ...
                                    <li>
                                        <BButton @click="console.log(ageCategory.id);store.getCategory(ageCategory.id)" class="btn btn-subtle-secondary btn-icon btn-sm">
                                            <i class="ph-pencil"></i>
                                        </BButton>
                                    </li>
                                    <li>
                                        <BButton @click="console.log(ageCategory.id);store.deleteCategorys(ageCategory.id)" class="btn btn-subtle-danger btn-icon btn-sm remove-item-btn">
                                            <i class="ph-trash"></i>
                                        </BButton>
                                    </li>
                                </ul>
                ...
                <div class="noresult" v-if="filteredAgeCategorys.length < 1">
                                <div class="text-center py-4">
                                    <i class="ph-magnifying-glass fs-1 text-primary"></i>
                                    <h5 class="mt-2">Sorry! No Result Found</h5>
                                    <p class="text-muted mb-0">We've searched more than 6+ Age Categories We did not find any
                                        Age Categories details for you search.</p>
                                </div>
                            </div>
</div>
<BRow class="align-items-center mt-4 pt-3" id="pagination-element" v-if="filteredAgeCategorys.length >= 1">
    <BCol cols="sm">
        <div class="text-muted text-center text-sm-start">
            Showing <span class="fw-semibold">{{ paginatedAgeCategorys.length }}</span> of
            <span class="fw-semibold">{{ filteredAgeCategorys.length }}</span> Results
        </div>
    </BCol>
    <BCol sm="auto" class="mt-3 mt-sm-0">
        <div class="pagination-wrap hstack justify-content-center gap-2">
            ...
            <BLink class="page-item pagination-next" href="#" :disabled="page >= totalPages" @click="setPage(page + 1)">
                ...
                <BModal v-model="store.showModal1" hide-footer title="Edit Age Category" modal-class="fadeInRight" >
                    <BRow class="align-items-center g-4" >
                <BCol lg="6" class="me-auto">
                    <label for="Designationname" class="form-label">Age Category name <span class="text-danger">*</span></label>
                    <div class="position-relative ">
            ...
            <div class="modal-footer">
                ...
                <BButton  @click="store.updateAgeCategorys()"  variant="primary">Save Changes</BButton>
            </div>
        </BModal>
            <!-- </div> -->
        </BCardBody>
