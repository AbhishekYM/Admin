...
import { useCategory } from '/src/store/category.js';
...
const filteredCategories = computed(() => {
    let categories = store.categorys;
    if (searchQuery.value.trim()) {
        const query = searchQuery.value.toLowerCase();
        categories = categories.filter(categories => {
            return (
                categories.name.toLowerCase().includes(query) 
              );
        });
    }
            ...
            <BRow class="align-items-center g-2">
                ...
                <BCol md="auto">
                    <div class="hstack gap-2">
                        ...
                        <BButton variant="primary" @click="store.modalShow17 = !store.modalShow17"> Add Bird Categorys </BButton>
                    </div>
                </BCol>
            </BRow>
        </BCardHeader>
        <BModal v-model="store.modalShow17" hide-footer title="Add Bird Categorys" modal-class="fadeInRight" >
          <BForm>    
        ...
        <BCardBody class="mt-3">
            <div class="table-responsive table-card">
                <BTableSimple class="table-centered align-middle table-custom-effect table-nowrap mb-0">
                    ...
                    <BTbody>
                        <BTr v-for="(designation, index) in paginatedCategories" :key="index">
                            ...
                            <BTd class="user_name">{{ designation.name }}</BTd>
                              <BTd>
                                <ul class="d-flex gap-2 list-unstyled mb-0">
                                    ...
                                    <li>
                                        <BButton @click="console.log(designation);store.deleteCategorys(designation.id)" class="btn btn-subtle-danger btn-icon btn-sm remove-item-btn">
                                            <i class="ph-trash"></i>
                                        </BButton>
                                    </li>
                                </ul>
                ...
                <div class="noresult" v-if="filteredCategories.length < 1">
...
<BRow class="align-items-center mt-4 pt-3" id="pagination-element" v-if="filteredCategories.length >= 1">
    <BCol cols="sm">
        <div class="text-muted text-center text-sm-start">
            Showing <span class="fw-semibold">{{ paginatedCategories.length }}</span> of
            <span class="fw-semibold">{{ filteredCategories.length }}</span> Results
        </div>
    </BCol>
    <BCol sm="auto" class="mt-3 mt-sm-0">
        <div class="pagination-wrap hstack justify-content-center gap-2">
            ...
            <BLink class="page-item pagination-next" href="#" :disabled="page >= totalPages" @click="setPage(page + 1)">
                ...
                <BModal v-model="store.showModal1" hide-footer title="Edit Bird Categorys" modal-class="fadeInRight">
                    <BRow class="align-items-center g-4" >
                <BCol lg="6" class="me-auto">
                    <label for="Category name" class="form-label">Category name <span class="text-danger">*</span></label>
                    <div class="position-relative ">
